...
- Staging smoke runner: PASS prepared on 2026-08-31 13:36 EDT. Added `scripts/hosted_staging_smoke.py`, covered it with regression test `test_hosted_staging_smoke_print_plan_lists_all_release_lanes`, and regenerated the seed package manifest so the host runs one command after container start: `./.venv/bin/python scripts/hosted_staging_smoke.py http://<staging-host>`. Fresh seed verification passed with DB SHA-256 `82a140b363bed8c675278be139f103bede56d6b8f3a575ecf7c079a658d29c4a`, source DB size 26,484,736 bytes, package size 4,540,759 bytes, 345 transcript rows, and required cached media verified: `v34Eg12mhDM` 90, `dQw4w9WgXcQ` 75, `SXHMnicI6Pg` 15, `aircAruvnKk` 15.
