...
- Quote-integrity JSON proof archive: PASS. `scripts/phase3_quote_integrity_smoke.py` now accepts `--out` and has regression coverage in `tests/test_phase3_quote_integrity_smoke_script.py`. Public proof saved to `evidence/phase3-quote-integrity-report.json`: record `7050f8734357`, analysis `cd5b57b3af5f`, 4 quote/source pairs checked, all exact matches, all sources found in transcript, Markdown download HTTP 200 / 1,145 bytes.
- Source-supported `content_assets_100`: PASS. Public proof saved to `evidence/phase3-100-assets-quality-report.json`: final post-reload record `3a707611c4a3`, analysis `d5a7ee23e999`, 100 assets, 100 unique bodies, 100 timestamped assets, max sentence-frame repeat 1, no invented quote-card formatting, and early/middle/late coverage from 1s to 1,602s. Assets now include the supporting source excerpt in each item.
- Citation coverage and public phase smokes: PASS. `evidence/phase3-citation-coverage-report.json` now shows all 17 outputs checked, `content_assets_100` with 100 assets and 100 matching transcript timestamps, combined analysis `dbe015b821fb`, Markdown download HTTP 200 / 52,822 bytes. Full suite returned 82 passed; Phase 1 health passed; Phase 2 release gate returned `all_ok=true`; Phase 3 UI contract returned combined analysis `f45024511c6f` with Markdown download HTTP 200 / 49,733 bytes.
...
- Regression suite and public phase smokes: PASS. Added `tests/test_phase3_long_analysis_smoke_script.py`; targeted script test returned 1 passed; full suite returned 75 passed; Phase 1 health returned cache-hit PASS for regression/control; Phase 2 release gate returned `all_ok=true`; Phase 3 UI contract returned combined analysis `364c945e44a1` with authenticated Markdown download HTTP 200 / 41,227 bytes.
...
Current evidence:
- 2026-08-31 17:26 EDT: Phase 3 long-transcript combined-analysis download privacy PASS. `scripts/phase3_long_analysis_smoke.py https://epic-transcript.robyncrane.com` now verifies unauthenticated long combined-analysis Markdown download is blocked before owner-authenticated download is accepted. Public proof: job `2a7020e81b9e`, cached two-hour record `837a9891f74d`, 19,298 words, 1,446 segments, duration 7,244s, combined analysis `61946223d75f`, 48,828 chars, latest timestamp 7,242s, unauthenticated download HTTP 403, owner-authenticated Markdown download HTTP 200 / 48,848 bytes. Full suite returned 75 passed.
