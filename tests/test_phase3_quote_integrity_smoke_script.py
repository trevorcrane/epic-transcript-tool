...
def load_smoke():
    spec = importlib.util.spec_from_file_location(
        "phase3_quote_integrity_smoke_under_test",
        ROOT / "scripts" / "phase3_quote_integrity_smoke.py",
    )
...
def test_parse_args_accepts_output_report_path(tmp_path):
    module = load_smoke()
    out = tmp_path / "phase3-quote-report.json"
...
def test_write_report_persists_same_json_that_is_printed(tmp_path, capsys):
    module = load_smoke()
    report = {"ok": True, "quote_count_checked": 3}
    out = tmp_path / "phase3-quote-report.json"

    module.write_report(report, out)

    expected = '{\n  "ok": true,\n  "quote_count_checked": 3\n}\n'
